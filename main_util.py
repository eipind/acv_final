...
def compute_edges(image, sigma=3):
...
def compute_votes(edges, model, threshold_inlier=5):
    vp = model[:2] / model[2]
...
def ransac_vanishing_point(edges, num_ransac_iter=2000, threshold_inlier=5):
    locations, directions, strengths = edges
...
def reestimate_model(model, edgelets, threshold_reestimate=5):
    ...
    lines = edge_lines((locations, directions, strengths))

    a = lines[:, :2]
    b = -lines[:, 2]
    est_model = np.linalg.lstsq(a, b, rcond=-1)[0]
...
def vis_edges(image, edgelets, show=True):
